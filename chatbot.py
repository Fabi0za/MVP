...
def train_chatbot(agent_chatbot, customer_chatbot):
    # Train the chatbots
    try:
        #Technically there could be two different chatcorpus
...
def create_chatbot():
    # Create a new chatbot for the agent
    agent_chatbot = ChatBot('AgentFinFluent')

    # Create a new chatbot for the customer
...
def customer_chatbot_page():
    return render_template("customer_chatbot.html")
...
def get_agent_bot_response():
    ...
    # If the agent enters "None", return a special message
    if user_text.lower() == "none":
        response = 'Proceeding without loading customer information.'
        return response
    # Check if the user input is a client number
    elif user_text in customer_data:
        # If it is, return the corresponding customer data
